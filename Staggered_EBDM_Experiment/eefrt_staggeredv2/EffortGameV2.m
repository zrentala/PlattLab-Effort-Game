...
runFileSelect = input('Please input trial number: ');
...
% for i = 1:1
    ...
    effort = effortVec(i);
       ...
       %Epoch 2
        ...
        DrawFormattedText(win,'ACCEPT: Y, REJECT: N',Xres-800,Yres-100,White);
        if effortDelay == 1
            DrawFormattedText(win,'EFFORT IN 30 DAYS',centerspot-200,900,White);
        end
        Screen(win, 'Flip');
...
%% Button Pressing task
            ...
            while (currentTime < startTime+timeDelta) && (goalReachedCount2 < ((currentEffort))) %While the time has not yet expired
                ...
                Screen('DrawText', win, strcat('Push:', 32, 32 , KbName(HARD),' until bar is', 32, num2str((effortVec(i)*100)),'% full with your non dominant pinky.'),600, 700, [255,255,255]); %Show reminder.
        ...
        ITIOffset = GetSecs-runStart;
         ...
         if effortDelay == 2
            ...
            fprintf(dataFile,'%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f \n',...
                ...
                epoch1press(i),epoch1pressRT(i),epoch2press(i),epoch2pressRT(i),handedness,Condition,totalPresses);
         end
