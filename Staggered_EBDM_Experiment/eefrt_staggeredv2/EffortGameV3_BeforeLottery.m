...
%                 Screen('FrameOval', win, Green, [725; 150; 1175; 750],8,8);  %[left,top,right,bottom]
        ...
        %% Opponent/Teamate screen
         ...
         compChance = rand;
            ...
            % Chooses text
            if chooseChance >= 0.5 
                line1 = [other " CHOICE: EFFORT"];
                %win chance
                % complete text
                if compChance >= 0.5
                    line2 = [other ' RESULT: WIN $' num2str(reward)];
                else
                    line2 = [other ' RESULT: FAILURE'];
                end 
            else
                line1 = [other ' CHOICE: NO EFFORT'];
                line2 = '';
            end
            DrawFormattedText(win,[line1 newline line2],'center','center',White); %['$' num2str(reward)]
     Screen(win, 'Flip'); 
    WaitSecs(4);
